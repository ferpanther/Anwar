package 
{
	...
	public class FichePresenceAdmin extends MovieClip
	{
		public static const FICHESPA_SAVED:String = "fichespa_saved";
		public static const FICHESPA_NOTSAVED:String = "fichespa_notsaved";
		...
		private var session_txt:String = "08:00 - 17:00";
		...
		//---------------------------------------------------
		public function FichePresenceAdmin()
		{
			if (this.stage)
			{
				init();
		...
		*/
		...
		private function updateRows():void
		{
			...
			for (var f:int = 0; f < agents.length; f++)
			{
				...
				rows[f].uid.text = agents[f]["uid"];
				rows[f].entree.text = presenceA[f][curr_mm][curr_jj - 1][session]["entree"];
				rows[f].sortie.text = presenceA[f][curr_mm][curr_jj - 1][session]["sortie"];
				rows[f].obs.text = presenceA[f][curr_mm][curr_jj - 1][session]["obs"];
		...
		public function save():void
		{
			...
			gw.call("Anwar.addFichePresenceA", save_responder, curr_jj, curr_mm, curr_yyyy, session, escape(new_fiche));
		}
		//------------------------------------------------
		function onSaveResult(reponse:Object):void
		{
			dispatchEvent(new Event(FichePresenceAdmin.FICHESPA_SAVED));
		}
		//------------------------------------------------
		function onSaveFault(reponse:Object):void
		{
			dispatchEvent(new Event(FichePresenceAdmin.FICHESPA_NOTSAVED));
		}
		//------------------------------------------------
